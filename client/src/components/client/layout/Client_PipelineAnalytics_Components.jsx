...
import {
  CLIENT_PIPELINE_ANALYTICS_OWNERS,
  CLIENT_PIPELINE_ANALYTICS_SOURCES,
  CLIENT_PIPELINE_DATE_RANGES,
  formatCurrency,
  formatShortCurrency,
} from "../../../services/clientPipelineAnalytics";
...
export function ClientPipelineAnalyticsHeader({
  filters,
  owners = [],
  sources = [],
  onChange,
  ...
  const ownerOptions =
    owners.length > 0 ? owners : CLIENT_PIPELINE_ANALYTICS_OWNERS;

  const sourceOptions =
    sources.length > 0 ? sources : CLIENT_PIPELINE_ANALYTICS_SOURCES;
    ...
    <div className="flex flex-col gap-4 border-b border-[var(--border-color)] pb-6 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
          Workspace funnel health, conversion analysis, forecasting, and sales
          velocity.
        </p>
      </div>
      ...
      <div className="grid w-full gap-3 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-3 shadow-sm lg:w-auto lg:min-w-[620px] lg:grid-cols-[1fr_1fr_1fr_auto]">
        ...
        >
          {CLIENT_PIPELINE_DATE_RANGES.map((range) => (
...
export function ClientPipelineAnalyticsTabs({ section, setSection }) {
  return (
...
export function ClientPipelineKPICards({ kpis }) {
  const cards = [
    {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
      {cards.map((card) => {
        ...
        return (
          <div key={card.label} className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] shadow-sm flex flex-col p-5">
...
export function ClientPipelineOverviewSection({
  funnelData,
  sourceAnalytics,
  forecasting,
  insights,
...
export function ClientPipelineFunnelSection({ funnelData, stagePerformance }) {
  return (
    <div className="space-y-4">
      <FunnelChart data={funnelData} />
...
export function ClientPipelineStagesSection({ stagePerformance }) {
  return <StagePerformanceTable data={stagePerformance} />;
}

export function ClientPipelineForecastSection({ forecasting }) {
  return (
...
export function ClientPipelineSourcesSection({ sourceAnalytics, insights }) {
...
function FunnelChart({ data = [] }) {
  ...
  function getStageColor(stage) {
    ...
    if (s.includes("discover") || s.includes("lead")) return "bg-[var(--brand-cyan)]"; // Initial phase
    if (s.includes("qualif") || s.includes("meet")) return "bg-blue-400"; // Developing phase
    if (s.includes("propos") || s.includes("negotiat")) return "bg-amber-500"; // Critical phase
    if (s.includes("won") || s.includes("success")) return "bg-[var(--success)]"; // Won phase
    return "bg-[var(--brand-gold)]"; // Fallback/default core value
  }
    ...
    <div className={`${panelClass} p-5`}>
      ...
      <div className="space-y-4">
        {data.map((item, index) => {
          ...
          const barColor = getStageColor(item.stage);
...
function StagePerformanceTable({ data = [] }) {
  ...
  return (
    <div className={`${panelClass} overflow-hidden`}>
      ...
      <div 
        className="custom-scrollbar overflow-x-auto transition-colors"
...
export function ClientPipelineAnalyticsLoadingState() {
  return (
...
export function ClientPipelineAnalyticsErrorState({ message, onRetry }) {
  return (
