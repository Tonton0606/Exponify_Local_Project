...
import {
  CLIENT_LEADERBOARD_BADGES,
  CLIENT_PERFORMANCE_BADGE_STYLES,
  formatCurrency,
...
export function ClientTeamKPICards({ kpis }) {
  const cards = [
    {
      label: "Top Performer",
...
export function ClientLeaderboardPodium({ data, onSelect }) {
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {data.map((person) => {
        const style =
          CLIENT_PERFORMANCE_BADGE_STYLES[person.rank] ||
          CLIENT_PERFORMANCE_BADGE_STYLES.default;
...
export function ClientLeaderboardTable({ data, onRowClick }) {
  return (
            ...
            {data.map((person) => (
              ...
              >
                <td className="px-5 py-3">
                  <span
                    className={
                      CLIENT_PERFORMANCE_BADGE_STYLES[person.rank]?.className ||
                      CLIENT_PERFORMANCE_BADGE_STYLES.default.className
                    }
                  >
                    #{person.rank}
                  </span>
                ...
                <td className="px-5 py-3">
                  <div className="flex flex-wrap gap-1">
                    {(person.badges || []).map((badge) => {
                      const style = CLIENT_LEADERBOARD_BADGES[badge];
...
export function ClientSalespersonDrawer({ person, onClose }) {
        ...
        <div className="p-6">
          {tab === "summary" && (
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <SummaryCard
                label="Pipeline"
                value={formatCurrency(person.pipelineValue)}
              />
            </div>
          )}
...
export function ClientActivityBreakdownChart({ data }) {
  return (
              ...
              <div key={label} className="mb-3 flex items-center gap-3">
                ...
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--hover-bg)]">
                  <div
                    ...
                    style={{
                      width: `${Math.min(Number(value || 0) * 3, 100)}%`,
                    }}
                  />
                </div>
